...
### Breaking changes
- Removed MIPI-DSI from display configuration structure - use `lvgl*port*add*disp*dsi` instead

## 2.1.0

### Features
- Added LVGL sleep feature: The esp*lvgl*port handling can sleep if the display and touch are inactive (only with LVGL9)
...
- Added blocking wait for LVGL task stop during esp*lvgl*port de-initialization https://github.com/espressif/esp-bsp/issues/277
- Added missing esp*idf*version.h include
