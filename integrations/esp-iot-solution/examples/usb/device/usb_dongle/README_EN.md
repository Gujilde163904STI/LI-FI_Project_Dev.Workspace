...
Refer to `soc/usb*pins.h` to find the real GPIO number of **USBPHY*DP*NUM** and **USBPHY*DM_NUM**.
...
![tinyusb*config](./*static/tinyusb_config.png)

![uart*config](./*static/uart_config.png)
...
- OTA*BIN*PATH is the upgrade firmware
...
- VendorID is USB vendor ID, the default value 0x303A
- Read firmware from device into OTA*BIN*PATH
...
3. Open the command window, run the following commands using `dfu-util.exe`.
   ...
   ```
   dfu-util.exe -d <VendorID> -a 0 -D <OTA*BIN*PATH>
   ```
