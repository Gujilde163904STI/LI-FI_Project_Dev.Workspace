...
* Fixed test*led*indicator.c build in C++ error
* Upgraded the display effect of color gradient.

## v0.9.1 - 2024-1-17

* Resolve the issue of calling preempt*stop with a specified blink*type that is not running, causing the current preemptive blink to stop.
...
* Resolve the error with 'is*active*level_high' in LEDC RGB type.

### Break Change

* Remove `led*indicator*strips*config*t:is*active*level_high`

* Remove `led*indicator*get*handle`, the query of 'led*indicator handle' through GPIO number and LEDC channel is no longer supported.
...
* Renamed function `get*current*fade*value` to `get*brightness`.
...
* Support LED RGB type by driver [led*strip](https://components.espressif.com/components/espressif/led*strip)
