...
- Each class has its own pair of header and implementation
  ...
  `my*module` is located at `path/to/my*module/riot/example/my_class.hpp`
  and the source file at `path/to/my*module/my*class.cpp`.

- All header files should use `#define` guards to prevent multiple inclusion.
  The symbol name is `<RELATIVE>*<PATH>*<TO>*<FILE>*HPP`.
...
- Break constructor initializers after the comma, use four spaces for
  ...
  other*class::other*class() : name*("tommy"), buddy*("michael") {
      // ok
  }
  ```
...
- Break function arguments after the comma for both declaration and invocation:
  ...
  ```cpp
  intptr*t channel::compare(const abstract*channel* lhs,
...
- Note that this is not necessary when simply defining a type alias.
  ...
  ```cpp
  using response*handle*type = response_handle<Subtype, message,
                                               ResponseHandleTag>;
  ```
...
- Macro names use the form `RIOT*<COMPONENT>*<NAME>`.
