...
- [Security object](http://www.openmobilealliance.org/tech/profiles/LWM2M*Security-v1*0.xml)
- [Server object](http://www.openmobilealliance.org/tech/profiles/LWM2M*Server-v1*0.xml)
- [Device object](http://www.openmobilealliance.org/tech/profiles/LWM2M*Device-v1*0_3.xml)
...
`lwm2m.h` as `CONFIG*LWM2M*BSSERVER_PORT`.
...
2. Fill in the name of the device, it **should** match the one set as `CONFIG*LWM2M*DEVICE_NAME`,
   in `objects/device.h`.
...
It can be set via `menuconfig` or the environmental variable `LWM2M*SERVER*URI`. It should be
reachable from the node, e.g. either running on native with a tap interface or as a mote connected
to a
[border router](https://github.com/RIOT-OS/RIOT/tree/master/examples/networking/gnrc*borader*router).

Also, if a bootstrap server is being used, it should be configured in the application via
`menuconfig` or setting the environmental variable `LWM2M*SERVER*BOOTSTRAP` to 1. This information
...
- The lwm2m client adaptation debug can be enabled by setting `ENABLE_DEBUG` in
  `lwm2m*client.c` and `lwm2m*client_connection.c` to 1
- The wakaama internal logging can be enabled by setting
  `CONFIG*LWM2M*WITH_LOGS`, either on `menuconfig` or by adding it to CFLAGS
  (`CFLAGS += -DCONFIG*LWM2M*WITH_LOGS`).
