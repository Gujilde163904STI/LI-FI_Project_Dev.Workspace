# gnrc*border*router using automatic configuration
...
Use `WIFI*SSID="SSID" WIFI*PASS="password"` in your `make` command to set your WiFi's
...
To compile and flash `gnrc*border*router` to your board:
...
For instance, if you use the [`gnrc*networking`](https://github.com/RIOT-OS/RIOT/tree/master/examples/networking/gnrc/gnrc*networking) example on the mote, you can
...
# gnrc*border*router with manual config
...
Then you can flash the `gnrc*border*router` example on your board:
...
# gnrc*networking*border_router with SLIP
