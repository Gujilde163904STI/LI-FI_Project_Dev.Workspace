...
looks for the macros `BOARD*BLACKLIST`, `BOARD*WHITELIST`, and
`BOARD*INSUFFICIENT*MEMORY`. Any board name that is not included in
...
system evaluates `BOARD*WHITELIST` first and then `BOARD*BLACKLIST`. The
`BOARD*INSUFFICIENT*MEMORY` macro is similar to `BOARD_BLACKLIST` but will
build in any case. A CI system can use the information provided by the
`BOARD*INSUFFICIENT*MEMORY` macro to skip the linking step in the build
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~ {.mk}
ifneq (,$(filter arch*esp,$(FEATURES*USED)))
  USEMODULE += esp_wifi
endif
...
`EXTERNAL*MODULE*DIRS` and `EXTERNAL*BOARD*DIRS` variables to specify
...
Note that the make variable (here `USEMODULE*INCLUDES*my_module`) must be unique
