...
the variable `KCONFIG*ADD*CONFIG`, which will be applied *after* default CPU and
board configurations, `app.config` and `user.config`. This means that they will
have priority.

Let's say that the `SOCK*UTIL*SCHEME*MAXLEN` symbol in `sock*util` module needs
...
the [tests/build*system/kconfig](https://github.com/RIOT-OS/RIOT/tree/master/tests/build*system/kconfig)
application.
...
`RIOT*CONFIG*`. To achieve the same configuration exemplified in
@ref configure-using-files, e.g., you could also use

~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ {.sh}
RIOT*CONFIG*KCONFIG*MODULE*SOCK_UTIL=1 \
RIOT*CONFIG*SOCK*UTIL*SCHEME_MAXLEN=24 \
    make
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
- `CPU_MODEL`: The specific identifier of the used CPU, used for some CPU
               ...
               layouts.
- `CPU*FAM`: An intermediate identifier between CPU and CPU*MODEL that
...
from more specific to less specific. This means that a `CPU*MODEL*<model>`
symbol usually would select the correspondent `CPU*FAM*<family>` symbol,
which would in turn select the `CPU*CORE*<core>`. This may change in some cases
where `CPU*COMMON*` symbols are defined to avoid repetition. For convenience and
if it makes sense within a CPU vendor family, it's also allowed to use
intermediate grouping levels, like `CPU*LINE*<xxx>` used for STM32.
...
`CPU*MODEL*<model>` symbol that corresponds to the CPU model present on the
...
model this in Kconfig. Symbols for common boards must have the `BOARD*COMMON*`
...
there are two Makefile variables developers should use: `KCONFIG*CPU*CONFIG` for
sources added by the CPU or common CPU directories, and `KCONFIG*BOARD*CONFIG`
...
| `BOARD*COMMON*` | Used for common symbols used by multiple boards |
| `CPU*ARCH*` | Models a CPU architecture |
| `CPU*COMMON*` | Used for common symbols used by multiple CPUs |
| `CPU*CORE*` | Models a CPU core |
| `CPU*FAM*` | Models a family of CPUs |
| `CPU*MODEL*` | Models a particular model of CPU |
...
can then make use of IS*ACTIVE macro from kernel*defines.h with C conditionals
...
'GNRC*IPV6*MSG*QUEUE*SIZE' expressed as the power of two. These may be modelled
