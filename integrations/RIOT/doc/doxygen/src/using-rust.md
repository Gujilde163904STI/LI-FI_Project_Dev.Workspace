...
Support is indicated in the `has*rust*target` feature,
and tested for in applications using the Makefile line
`FEATURES*REQUIRED += rust*target`.
...
which gets used by default if `BUILD*IN*DOCKER=1` is set.
...
[riot*wrappers::mutex::Mutex]: https://rustdoc.etonomy.org/riot*wrappers/mutex/struct.Mutex.html
...
that then depends on `rust*riotmodules*standalone`,
...
* When neither casting nor type detection is sufficient,
  ...
  [See the coap*request*ctx_t change] for an example.
...
* When functions are moved from being static and not being static,
  their names go from `riot*sys::inline::name` to `riot*sys::name` (or vice versa).

  riot-sys [has a list] of items that are always publicly exported directly as `riot_sys::name`;
  just add the function there.

  If non-generic types are referenced in them, they go from `riot*sys::inline::my*type*t`  to `riot*sys::my*type*t`.
...
[See the coap*request*ctx_t change]: https://github.com/RIOT-OS/rust-riot-wrappers/pull/4/files
