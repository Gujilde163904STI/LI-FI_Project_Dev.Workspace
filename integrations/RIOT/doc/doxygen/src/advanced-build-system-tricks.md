...
+ `RIOT*MAKEFILES*GLOBAL_PRE`: files parsed before the body of
  `$RIOTBASE/Makefile.include`
+ `RIOT*MAKEFILES*GLOBAL_POST`: files parsed after the body of
  `$RIOTBASE/Makefile.include`
...
`dependencies*info*board_name` file:
...
to a `dependencies*info-boards-supported*board_name` file:

~~~~~~~~~~~~~~~~~~~
BOARDS=board*name DEPENDENCY*DEBUG=1 make info-boards-supported
~~~~~~~~~~~~~~~~~~~
...
`dist/tools/buildsystem*sanity*check/save*all*dependencies*resolution*variables.sh`
...
be available. The target supports using docker via the `BUILD*IN*DOCKER=1`
variable.
