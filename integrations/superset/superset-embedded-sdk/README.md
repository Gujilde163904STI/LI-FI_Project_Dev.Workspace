...
* Set a strong password in configuration variable `GUEST*TOKEN*JWT_SECRET` (see configuration file config.py). Be aware that its default value must be changed in production.
...
The agent making the `POST` request must be authenticated with the `can*grant*guest_token` permission.

Within your app, using the Guest Token will then allow authentication to your Superset instance via creating an Anonymous user object.  This guest anonymous user will default to the public role as per this setting `GUEST*ROLE*NAME = "Public"`.
...
To do this, you should update the `GUEST*TOKEN*JWT_SECRET`
in the Superset [config.py](https://github.com/apache/superset/blob/master/superset/config.py). Also set the
`GUEST*TOKEN*JWT_AUDIENCE` variable that matches what is set for the `aud` in the JSON payload:
