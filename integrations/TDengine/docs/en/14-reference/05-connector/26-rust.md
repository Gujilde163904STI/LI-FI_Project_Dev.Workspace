...
| v0.9.2                 | STMT: WebSocket to get tag*fields, col*fields.                                                        | 3.0.7.0 and higher |
...
- `fn query*with*req*id<T: AsRef<str>>(&self, sql: T, req*id: u64) -> RawResult<Self::ResultSet>`
...
- `fn deserialize<T>(&mut self) -> Map<IRowsIter<'*, Self>, fn(*: Result<RowView<'_>, Error>) -> Result<T, Error>>`

  - **Interface Description**: Deserialize row data.
  - **Generic Parameters**:
    - `T`: Target type, must implement `DeserializeOwned`.
  - **Return Value**: Map of the deserialization results, type `Map<IRowsIter<'*, Self>, fn(*: Result<RowView<'_>, Error>) -> Result<T, Error>>`.

- `fn to*rows*vec(&mut self) -> Result<Vec<Vec<Value>>, Error>`
...
- `pub fn to*c*field(&self) -> c*field*t`
...
- `fn init*with*req*id(taos: &Q, req*id: u64) -> RawResult<Self>`
...
- `fn set*tbname*tags<S: AsRef<str>>(&mut self, name: S, tags: &[Value]) -> RawResult<&mut Self>`
...
- `fn commit*offset(&self, topic*name: &str, vgroup_id: VGroupId, offset: i64) -> RawResult<()>`
...
- `fn offset*seek(&mut self, topic: &str, vg*id: VGroupId, offset: i64) -> RawResult<()>`
