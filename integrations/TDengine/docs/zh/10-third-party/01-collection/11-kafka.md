...
2. `connection.database.prefix`：当 connection.database 为 null 时，目标数据库的前缀。可以包含占位符 '$\{topic}'。比如 kafka*$\{topic}, 对于主题 'orders' 将写入数据库 'kafka*orders'。默认 null。当为 null 时，目标数据库的名字和主题的名字是一致的。
