...
1. JDBC URL 通过环境变量配置，例如：`export TDENGINE*JDBC*URL="jdbc:TAOS-WS://localhost:6041?user=root&password=taosdata"`
2. Kafka 集群地址通过环境变量配置，例如： `KAFKA*BOOTSTRAP*SERVERS=localhost:9092`
...
3. 在开发环境中配置环境变量 `TDENGINE*JDBC*URL`。如果已配置了全局的环境变量 `TDENGINE*JDBC*URL` 可跳过这一步。
4. 如果要运行 Kafka 示例，需要设置 Kafka 集群地址的环境变量 `KAFKA*BOOTSTRAP*SERVERS`。
