...
Before testing, please make sure the building operation with option `-DBUILD*TOOLS=true -DBUILD*TEST=true -DBUILD_CONTRIB=true` has been done, otherwise execute commands below:
...
Create a file in `tests/system-test` containing each functional directory and refer to the use case template `tests/system-test/0-others/test*case*template.py` to add a new test case. 
...
New cases can be added by updating the value of `commands` variable in `test*smoking*selfhost.sh`.
...
1. Add a function, such as `TaskCreateNewFunction` in `pytest/crash*gen/crash*gen_main.py`.
2. Integrate `TaskCreateNewFunction` into the `balance*pickTaskType` function in `crash*gen_main.py`.
