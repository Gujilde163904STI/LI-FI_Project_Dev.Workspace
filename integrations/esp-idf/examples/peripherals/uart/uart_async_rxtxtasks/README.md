...
The `RXD*PIN` and `TXD*PIN` which are configurable in the code (by default `GPIO4` and `GPIO5`) need to be shorted in
...
If you do not see any output from `RX*TASK` then check if you have the `RXD*PIN` and `TXD_PIN` pins shorted on the board.
