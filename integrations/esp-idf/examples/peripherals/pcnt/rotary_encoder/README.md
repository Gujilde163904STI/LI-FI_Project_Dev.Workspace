...
The GPIO used by the example can be changed according to your board by `EXAMPLE*EC11*GPIO*A` and `EXAMPLE*EC11*GPIO*B` in [source file](main/rotary*encoder*example_main.c).

### Build and Flash

By configuring one of the EC11 GPIO (e.g. `EXAMPLE*EC11*GPIO*A`) as a wake up source, you can make the rotary encoder wake the system from light sleep. This example can illustrate this feature if you enable the `EXAMPLE*WAKE*UP*LIGHT_SLEEP` from the menuconfig.
