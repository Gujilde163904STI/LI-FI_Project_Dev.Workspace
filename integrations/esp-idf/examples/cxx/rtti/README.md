...
By default, RTTI support is disabled in ESP-IDF. It can be enabled using `CONFIG*COMPILER*CXX_RTTI` configuration option.

In this example, `sdkconfig.defaults` file sets `CONFIG*COMPILER*CXX_RTTI` option. This enables compile time support for RTTI (`-frtti` compiler flag).
