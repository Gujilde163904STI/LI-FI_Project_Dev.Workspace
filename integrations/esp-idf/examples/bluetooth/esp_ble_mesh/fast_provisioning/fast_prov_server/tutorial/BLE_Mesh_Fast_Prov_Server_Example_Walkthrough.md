...
The cache data is required, so a node can change its role to become a provisioner. During this process, the `esp*ble*mesh*set*fast*prov*info` and `esp*ble*mesh*set*fast*prov*action` APIs are called.
