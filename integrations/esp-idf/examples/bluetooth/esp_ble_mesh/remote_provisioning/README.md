...
Please refer to this [tutorial](tutorial/BLE*Mesh*Remote*Provisioning*Example_Walkthrough.md) for detailed instructions on how to run it.
...
## Example Output
When a device that runs the `rpr*server` joins the network, the device that runs the `rpr*client` will have the following log output:
