...
Program should run correctly without needing any special configuration. However, users can disable `CONFIG*ESP*TASK*WDT*INIT` which will prevent the TWDT from being automatically initialized on startup. If disabled, the example will manually initialize the TWDT.
...
Users can comment out any of the `esp*task*wdt*reset()` or `esp*task*wdt*reset_user()` calls to trigger the TWDT, which in turn will result in the following output:
