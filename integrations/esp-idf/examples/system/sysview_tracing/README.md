...
Check the full example code [sysview*tracing](main/sysview*tracing.c) that when compiled in dual core mode reproduces the described problem on both cores. Below is the output of example compiled in single core mode. It shows that task misses several events:
...
10.  Now you can navigate over user-defined messages and see when timer `TG1*T0*LEVEL` sends events (`SYSVIEW*EXAMPLE*SEND*EVENT*START/STOP` or `User Start/Stop(0)`) and when task `svtrace0` reads them (`SYSVIEW*EXAMPLE*WAIT*EVENT*START/STOP` or `User Start/Stop(1)`).
