...
1. There is a directory `fatfs*long*name_image` from which the FATFS filesystem image will be created.

2. Based on the RO/RW configuration either `fatfs*create*rawflash*image` or `fatfs*create*spiflash*image` respectively,
...
`FLASH*IN*PROJECT` specifies that the created image
...
The logic of the example is contained in a [single source file](./main/fatfsgen*example*main.c),
