...
1. Use an "all-in-one" `esp*vfs*spiffs_register` function to:
    - initialize SPIFFS,
...
* Configure SPIFFS settings under "SPIFFS Example menu". See note about `esp*spiffs*check` function on [SPIFFS Filesystem](https://docs.espressif.com/projects/esp-idf/en/latest/esp32/api-reference/storage/spiffs.html) page.
...
Here is an example console output. In this case `format*if*mount_failed` parameter was set to `true` in the source code. SPIFFS was unformatted, so the initial mount has failed. SPIFFS was then formatted, and mounted again.
