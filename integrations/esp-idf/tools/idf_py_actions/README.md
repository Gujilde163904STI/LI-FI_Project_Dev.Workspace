# idf.py extensions
Python modules (subdirectories and files) in this directory named `[your*extension]*ext` will be loaded as idf.py extensions.
If you want to provide extra extensions just provide `;` separated list of directories with extensions in  `IDF*EXTRA*ACTIONS_PATH`. Extensions will be loaded in alphanumeric order.
...
They should define a function `action*extensions(base*actions, project_path)` where:
...
Where function `global*callback(ctx, global*args, tasks)` accepts 3 arguments:
...
And `subcommand*callback(subcommand*name, ctx, args)` accepts 3 arguments:
