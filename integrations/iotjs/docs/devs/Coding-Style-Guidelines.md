...
### Type names
Use lower cases and underscore for struct names, and add prefix `iotjs*` and suffix `*t`.
...
For constructors and destructor, use names starting with `iotjs*mystruct**`.
Constructor function name should be either `iotjs*mystruct*create` or `iotjs*mystruct*initialize`,
