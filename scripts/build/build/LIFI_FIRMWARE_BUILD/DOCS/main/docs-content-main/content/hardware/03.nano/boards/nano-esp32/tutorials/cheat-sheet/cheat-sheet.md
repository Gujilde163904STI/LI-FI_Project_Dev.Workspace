...
- [IO MUX and GPIO Matrix (ESP32-S3 technical reference manual)](https://www.espressif.com/sites/default/files/documentation/esp32-s3*technical*reference*manual*en.pdf#iomuxgpio)
...
The ESP32 features an RGB LED that can be controlled with the `LED*RED`, `LED*GREEN` and `LED_BLUE` pin names. These pins are not accessible on the headers of the board, and can only be used for the RGB LED. 
