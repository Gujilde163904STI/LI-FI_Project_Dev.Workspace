...
- `input*pullup = Pin(pin, Pin.IN, Pin.PULL*UP` (define as input pull up)
- `input*pulldown = Pin(pin, Pin.IN, Pin.PULL*DOWN` (define as input pull down)
...
- `pin1 = Pin("PC6", Pin.OUT*PP, Pin.PULL*NONE)`
...
`bit*value = (variable >> bit*index) & 1`
...
`leftmost*bit = (variable >> leftmost*bit_index) & 1`

Reads the high-order (leftmost) bit of a variable.

**Parameters**
- `variable` - an integer variable with numeric value, e.g. `255`
- `leftmost*bit*index` - the leftmost bit, e.g. `7` in an 8-bit variable.
...
`map(value, in*min, in*max, out*min, out*max)`
...
`spi.write*readinto(data*out, data_in)`
