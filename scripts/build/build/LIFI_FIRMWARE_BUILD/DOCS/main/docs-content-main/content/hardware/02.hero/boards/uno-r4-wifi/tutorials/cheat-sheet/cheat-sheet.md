...
void setup() {
  ...
  RTC.begin();
  RTCTime mytime(30, Month::JUNE, 2023, 13, 37, 00, DayOfWeek::WEDNESDAY, SaveLight::SAVING*TIME*ACTIVE);
...
```arduino
FspTimer::force*use*of*pwm*reserved_timer();
```
...
```arduino
uint8*t gpt*timer*type = GPT*TIMER;
uint8*t agt*timer*type = AGT*TIMER;
```
...
//choose from BR*125k,BR*250k,BR*500k,BR*1000k 
...
```arduino
uint8*t const msg*data[] = {0xCA,0xFE,0,0,0,0,0,0};
memcpy((void *)(msg*data + 4), &msg*cnt, sizeof(msg_cnt));
CanMsg msg(CAN*ID, sizeof(msg*data), msg_data);
CAN.write(msg);
```
