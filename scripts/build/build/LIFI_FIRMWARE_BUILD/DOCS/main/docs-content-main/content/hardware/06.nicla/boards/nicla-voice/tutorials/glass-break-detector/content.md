...
- **flash*(os)*mcu:** Flashes only the MCU firmware.
- **flash*(os)*model:** Flashes the NDP firmware, DSP firmware (if not present), and updates the model.
- **format*(os)*ext_flash:** Erases the external flash memory.
...
For a new model deployment with manual approach, use the [Syntiant® uploader](assets/Syntiant*Uploader.zip) and replace the `ei*model.synpkg`, which you can find inside the downloaded folder with yours.
