...
![Cloning lmp-manifest repository](assets/git*clone*lmp-manifest.png)
...
![Git-repo finished sync](assets/repo*sync*finished.png)
...
***Support for `lmp-factory-image` is anticipated to improve continuously. Starting from image __version 888__, *`lmp-partner-arduino-image`* is now known as __`lmp-factory-image`__.***
...
![Setup Portenta X8 DISTRO](assets/x8*distro*setup.png)
...
- Reduce `BB*NUMBER*PARSE*THREADS` and `BB*NUMBER_THREADS` to `"4"`
...
![Portenta X8 Image finished compilation](assets/x8*build*finished.png)
...
![Flashing tools DISTRO setup](assets/tools*distro*setup.png)
...
![Checking copied files with file explorer](assets/docker*volume*explorer.png)
