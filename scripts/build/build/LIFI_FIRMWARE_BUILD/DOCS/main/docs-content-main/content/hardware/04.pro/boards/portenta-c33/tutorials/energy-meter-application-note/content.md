...
![Portenta C33 with a non-invasive current transformer](assets/C33*application*1.png)
...
![Application's electrical connection overview](assets/C33*application*4.png)
...
![Correct usage of a non-invasive current transformer](assets/C33*application*2.png)
...
![Working principle of a non-invasive current transformer](assets/C33*application*3.gif)
...
* Finally calls the `iot*cloud*setup()` function to set up the Arduino Cloud connection.
...
* It then updates the `cloud*Current` and `cloud*ApparentPower` variables with the local values of current and apparent power, then waits for a second before the next iteration.

### User Functions

The `iot*cloud*setup()` function groups the initial configuration process of the Arduino Cloud as follows:
...
The variables `sct*ratio`, `system*Voltage`, and `sample_Control` are used to synchronize the local values with the Arduino Cloud.
...
![Arduino Cloud Dashboard Example](assets/C33*desktop*app.gif)
...
![Arduino Cloud Mobile Dashboard Example](assets/C33*app*cloud_mobile.gif)
