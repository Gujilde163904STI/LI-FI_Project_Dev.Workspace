...
* Use camel case function names, not underscore. For example, **analogRead**, not **analog*read**. Or **myNewFunction**, not **my*new_function**. We've adopted this from Processing.org for readability's sake.

**LONG*CONSTANT*NAMES*FULL*OF_CAPS are hard to read.** Try to simplify when possible, without being terse.
