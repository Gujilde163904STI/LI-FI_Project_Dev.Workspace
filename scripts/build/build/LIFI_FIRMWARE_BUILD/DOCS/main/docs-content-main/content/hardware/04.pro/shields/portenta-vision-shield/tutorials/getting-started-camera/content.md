...
coverImage: assets/vs*ard*gs_cover.svg
difficulty: beginner
...
![Connecting the Portenta Vision Shield to Portenta](assets/vs*ard*gs*attach*boards.svg)
...
![Download the mbed core](assets/vs*ard*gs_core.png)
...
In the loop you need to capture each Frame and send it over a serial connection to the Processing sketch that will display the frames. You will use the `grab(uint8*t *buffer, uint32*t timeout=5000);` function to fetch the frame from the frame buffer and save it into your custom data buffer. 
...
![Create a processing sketch](assets/vs*ard*open*pde*sketch.png)
...
The first thing you can do inside this method is to update the timestamp when the last data was read. This is to detect and recover from a connection timeout. Then read the bytes from the `frameBuffer` array which you can do with the help of the [`readBytes()`](https://processing.org/reference/libraries/serial/Serial*readBytes*.html) method that returns the number of bytes read.
...
![Camera output on Processing](assets/vs*ard*frames_captured.png)
