...
In this guide, we will focus on the advanced ADC/DAC features, utilizing the [Arduino*AdvancedAnalog](https://github.com/arduino-libraries/Arduino*AdvancedAnalog) library. The examples found in this guide can be used to:
...
- [Arduino*AdvancedAnalog](https://github.com/arduino-libraries/Arduino*AdvancedAnalog) library.
...
- [Arduino*USBHostMbed5](https://github.com/arduino-libraries/Arduino*USBHostMbed5) library installed.
...
To read `.wav` files from the USB stick we are using the **Arduino*USBHostMbed5** library. It is important that the USB stick is formatted properly, and that we define its name in the sketch. In this case, we name it `USB*DRIVE`, and is defined like this: 
...
The following example plays a single `.wav` file named `AUDIO*SAMPLE.wav` from a USB mass storage device named `USB*DRIVE`.
...
You can download them from [this link](/resources/misc/giga*audio*examples.zip).
