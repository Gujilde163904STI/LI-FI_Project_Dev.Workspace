...
![When the board is successfully connected to the OpenMV IDE a green play button appears in the lower left](assets/por*openmv*board_connected.png)
...
Depending on how you hold the camera, you may want to play with the `set*vflip` and `set*hmirror` functions. To hold the board with the USB cable facing down, you will need to call `set*vflip(True)`. If you want the image to be displayed the same way as you see it with your eyes, you need to call `sensor.set*hmirror(True)`. Otherwise elements in the image such as text would be mirrored.
...
![LAB thresholds for an apple in the Threshold Editor](assets/lab*thresholds*apple.png)
...
![LAB thresholds for a banana in the Threshold Editor](assets/lab*thresholds*banana.png)
...
![Visualization of the blobs in the frame buffer preview](assets/blob*detection*example.png)
