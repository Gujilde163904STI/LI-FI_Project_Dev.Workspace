...
Your sketchbook is where your code files are stored. Arduino sketches are saved as `.ino` files, and must be stored in a folder of the exact name. For example, a sketch named `my*sketch.ino` must be stored in a folder named `my*sketch`.
