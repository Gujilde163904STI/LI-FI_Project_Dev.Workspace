...
`#define INCLUDE*FALLBACK*INDEX_HTM`
...
If you use the gzipped or `INCLUDE*FALLBACK*INDEX*HTM` options, please remember to rerun the `reduce*index.sh` script located in the `extras` subfolder and recompile the sketch after each change to the `index.html` file.
