...
* Remove quotes around `SETUPTOOLS*SCM*PRETEND_VERSION` recipe variable. (#317)
...
* Replace deprecated `run*command` function with `conda*cli`. (#310)
...
* Expand variables when creating `working*dir` and use `os.path.expanduser` logic for default `working*dir` on Windows. (#211 via #212)
...
* Add `app*user*model_id` field on Windows shortcuts to group taskbar icons together. (#127 via #133)
