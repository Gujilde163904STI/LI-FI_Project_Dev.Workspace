...
editing `BIT*RATE*DEFAULT` in `app/include/user_config.h`:
...
storing variables.
You can change this
either by uncommenting `LUA*NUMBER*64BITS` in `app/include/user_config.h`:
...
either by uncommenting `LUA*NUMBER*INTEGRAL` in `app/include/user_config.h`:
...
You may also edit `app/include/user*version.h`. The variable `USER*PROLOG` will be included in `NODE*VERSION*LONG`.
...
Display drivers and embedded fonts are compiled into the firmware image based on the settings in `app/include/u8g2*displays.h` and `app/include/u8g2*fonts.h`. See the [`u8g2` documentation](modules/u8g2.md#displays) for details.
