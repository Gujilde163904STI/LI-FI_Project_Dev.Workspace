...
by pointing `--@pico-sdk//bazel/config:PICO*CONFIG*HEADER` to a `cc_library`
that defines `PICO*BOARD` and either a `PICO*CONFIG_HEADER` define or a
...
`pico*uf2*aspect` enabled is the easiest way to create a UF2 for every ELF
firmware image.
