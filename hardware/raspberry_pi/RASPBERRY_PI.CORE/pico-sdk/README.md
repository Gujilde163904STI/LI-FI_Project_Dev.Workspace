...
1. Set up your project to point to use the Raspberry Pi Pico SDK
   ...
   * Or by cloning the SDK locally, but without copying `pico*sdk*import.cmake`:
       1. `git clone` this Raspberry Pi Pico SDK repository
...
1. Write your code (see [pico-examples](https://github.com/raspberrypi/pico-examples) or the [Raspberry Pi Pico-Series C/C++ SDK](https://rptl.io/pico-c-sdk) documentation for more information)
   ...
   Note this example uses the default UART for *stdout*;
...
1. Setup a CMake build directory.
   ...
   When building for a board other than the Raspberry Pi Pico, you should pass `-DPICO*BOARD=board*name` to the `cmake` command above, e.g. `cmake -DPICO*BOARD=pico2 ..` or `cmake -DPICO*BOARD=pico_w ..` to configure the SDK and build options accordingly for that particular board.
   ...
   cases also enables the use of additional libraries (e.g. wireless support when building for `PICO*BOARD=pico*w`) which cannot
...
1. You now have `hello*world.elf` to load via a debugger, or `hello*world.uf2` that can be installed and run on your Raspberry Pi Pico-series device via drag and drop.
