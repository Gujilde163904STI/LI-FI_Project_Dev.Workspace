...
* Add support for a bootloader watchdog
  ...
  watchdog is enabled by setting the BOOT*WATCHDOG*TIMEOUT=N (seconds)
  property in the bootlaoder config.
  The BOOT*WATCHDOG*PARTITION=P property can be set to pass a different
...
* dtoverlay: Fix node*is*enabled for implicit status
  The absence of a status property implies that a node is enabled. Update
  dtoverlay*node*is_enabled to match that behaviour.
  See: https://github.com/raspberrypi/firmware/issues/1970
* arm*loader: GET*CLOCKS: Set useful response length
...
* arm_loader: Correct some mailbox response lengths
  The GET*GENCMD*RESULT mailbox handler was setting the wrong response
  length, and GET*FIRMWARE*COMMIT*HASH and GET*FIRMWARE_VARIANT were not
  setting any length.
...
* Implement GET*BOARD*MAC_ADDRESS on Pi5
  The Pi 5 EEPROM implements a subset of the original mailbox properties.
  Add GET*BOARD*MAC_ADDRESS to the subset.
...
* os_check: cm5: Check for CM5 specific dtbs
  ...
  This avoids needing to put os*check=1 or specifying device*tree
...
* config*loader: Add support [boot*partition=N] as an expression filter
...
* pi5: Add config filter for simple boot variable expressions (experimental)
  ...
  which tests the partition, boot*count and boot*arg1 variables.
  Syntax (no spaces):
  ARG boot*arg1, boot*count or partition (EEPROM config stage only)
...
* Add memory barrier to the mbox handler
  ...
  possible, as handle*mbox*property always sets the top bit of the return
...
* arm*dt: Use dtoverlay*enable_node
  Convert the open-coded DT node status changes to use the new dtoverlay
  method dtoverlay*enable*node.
* dtoverlay: Add dtoverlay*enable*node
...
* Add DT /chosen property signed-boot boot.img hash
  ...
  device-tree /proc/device-tree/chosen/bootloader/boot*img*sha256 if
  signed boot is enabled.
* filesystem: GPT autoboot/reboot partition number fixes for Pi4 and older
* Fix problems when setting arm*freq*min=arm_freq and display clocks
  if performance governor is not enabled.

## 2025-01-14: Add set*reboot*order API (latest)

* Add set*reboot*order API and config.txt properties
  If set*reboot*order is defined in config.txt or set via vcmailbox
...
* Add an option to wait for the power button to be pressed before booting.
  If POWER*OFF*ON*HALT=1 and WAIT*FOR*POWER*BUTTON=1 in the bootloader
...
* Disable fan PWM before shutdown
  ...
  sleep (POWER*OFF*ON_HALT=0) to stop the fan spinning.
...
* arm*dt: enable*uart defaults to 0 on 2712
...
* Add net install to boot menu
  Press N (or shift).
* enable*uart: Require enable*uart=1 to enable RP1 UART console
...
## 2024-11-27: rp1fw: Add FIFO*STATE & DRAIN*TX, fix CAN*ADD*PROGRAM (default)

* rp1fw: Add FIFO*STATE & DRAIN*TX, fix CAN*ADD*PROGRAM
  ...
  fixes the CAN*ADD*PROGRAM implementation, which was fatally broken.
...
* Set POWER*OFF*ON_HALT defaults
  The default value for POWER*OFF*ON_HALT on CM5 and Pi 500 will be 1.
...
## 2024-11-05: NUMA - Add system*heap.max*order=0 when needed (latest)

* NUMA - Add system*heap.max*order=0 when needed - configure this
...
* Display the bootloader network-install UI for longer on a cold boot to make
  ...
  To revert to the previous behaviour remove NET*INSTALL*AT*POWER*ON=1
...
* arm*dt: Consult the hat*map for all HATs
* USB boot - ignore RP2 / RP3 MSD device in BOOTSEL mode.
* recovery.bin - Fix erase*eeprom to not block reboot*recovery
...
* Add enable*rp1*uart=1 to config.txt to initialise RP1 UART0 immediately
...
* bootloader: clock*2712: Remove restriction on arm*freq <= 3000
  See: https://github.com/raspberrypi/firmware/issues/1876
* arm*dt: Update max*current to match HAT value
* arm*dt: Remove unused legacy parameters (core*freq, arm*freq, uart0*clkrate and cache*line*size)
...
## 2024-02-14: Fix vcgencmd otp*dump and implement C(initial*turbo) (latest)
...
* Force PWM on 3V3 supply if cameras or HATs are connected or if power*force*3v3_pwm=1 in config.txt
  Resolves an image quality issue with the GS camera.
* Add support for C(arm*min*freq) < 1500 MHz (must be at >= 200 MHz)
...
## 2023-09-28: vcgencmd pmic*read*adcs fixes (automatic update)
