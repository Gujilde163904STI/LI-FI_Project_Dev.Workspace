...
#### Syntax
`gpio.serout(pin, start*level, delay*times [, cycle_num[, callback]])`
...
This function is not available if GPIO*INTERRUPT*ENABLE was undefined at compile time.
...
!!! Attention
	...
	This sub module is disabled by default. Uncomment `LUA*USE*MODULES*GPIO*PULSE` in `app/include/user_modules.h` before building the firmware to enable it.
