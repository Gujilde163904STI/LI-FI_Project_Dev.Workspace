...
![low-pass filter](../img/sigma*delta*audiofilter.png "low-pass filter for sigma-delta driver")
...
Also see [play*file.lua](../../lua*examples/pcm/play_file.lua) in the examples folder.
...
#### Parameters
`rate` sample rate. Supported are `pcm.RATE*1K`, `pcm.RATE*2K`, `pcm.RATE*4K`, `pcm.RATE*5K`, `pcm.RATE*8K`, `pcm.RATE*10K`, `pcm.RATE*12K`, `pcm.RATE*16K` and defaults to `RATE_8K` if omitted.
