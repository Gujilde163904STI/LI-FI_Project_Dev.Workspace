...
- `callback` The callback function to be invoked when the response has been received or an error occurred; it is invoked with the arguments `status*code`, `body` and `headers`. In case of an error `status*code` is set to -1.
