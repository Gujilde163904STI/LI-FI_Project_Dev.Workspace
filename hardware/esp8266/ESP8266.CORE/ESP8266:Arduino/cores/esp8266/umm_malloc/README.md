...
Copy the `umm*malloc*cfg*example.h` file to `umm*malloc_cfg.h` and
...
- `UMM*MALLOC*CFG*HEAP*ADDR` must be set to the symbol representing
  the starting address of the heap. The heap must be
  aligned on the natural boundary size of the processor.
- `UMM*MALLOC*CFG*HEAP*SIZE` must be set to the size of the heap.
...
the corresponding entries is `umm*malloc*cfg.h`:
...
- `void *umm*malloc( size*t size );`
- `void *umm*calloc( size*t num, size_t size );`
- `void *umm*realloc( void *ptr, size*t size );`
