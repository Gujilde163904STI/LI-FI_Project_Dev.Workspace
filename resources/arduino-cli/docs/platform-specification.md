...
[Boards Manager](package*index*json-specification.md) or manual installation to the *hardware* folder of Arduino's
...
In this example the property **recipe.c.o.pattern** will be set to **/tools/g++\*arm*none_eabi/bin/arm-none-eabi-gcc**,
...
- `{runtime.platform.path}`: is the absolute path of the [board platform](#platform-terminology) folder (i.e. the folder
  containing boards.txt) unless `{runtime.use*core*platform*path*for*runtime*platform_path}` is set to `true` in this
...
- `{build.fqbn}`: the FQBN (fully qualified board name) of the board being compiled for. The FQBN follows the format:
  `VENDOR:ARCHITECTURE:BOARD*ID[:MENU*ID=OPTION*ID[,MENU2*ID=OPTION_ID ...]]`
...
- `{build.library*discovery*phase}`: set to 1 during library discovery and to 0 during normal build. A macro defined
  ...
  intent, `-DARDUINO*LIB*DISCOVERY_PHASE` was added to `recipe.preproc.macros` during library discovery in Arduino
  Builder 1.5.3/Arduino CLI 0.10.0. That flag was replaced by the more flexible `{build.library*discovery*phase}`
  property.
...
- `{runtime.use*core*platform*path*for*runtime*platform_path}` support is available from Arduino CLI >=1.0.4.
...
[`arduino-cli core list`](commands/arduino-cli*core*list.md)'s output for the platform.<br> The **version** is currently
...
the compile are also archived into a static library named *core.a* using the **recipe.ar.pattern**.
...
- `{archive*file*path}`: fully qualified archive file (ex. "/path/to/core.a"). This property was added in Arduino IDE
...
`FILE*EXTENSION*x` must be replaced with the extension of the extracted file, for example the AVR platform needs two
...
- `{upload.maximum*data*size}`: available dynamic memory for global variables
...
If both **recipe.size.pattern** and **recipe.advanced*size.pattern** are present then **recipe.advanced*size.pattern**
...
(but not **recipe.output.tmp*file** and **recipe.output.save*file**) can be built concatenating the following
...
- `{includes}`: the list of include paths in the format "-I/include/path -I/another/path...."
- `{source_file}`: the path to the source file
- `{preprocessed*file*path}`: the path to the output file
...
Note that the `{preprocessed*file*path}` might point to (your operating system's equivalent) of `/dev/null`. In this
...
development software outputs a warning. In this case the macro defined at compile time will be `ARDUINO*AVR*UNO`.
...
For example we could declare a series of `upload*port.vid` and `upload*port.pid` properties for the Uno like so:
...
In this example, the Arduino Uno board needs the *standard* variant so the **build.variant** property is set to
*standard*:
...
bootloaders using external programmers. For example, *avrdude* is used for AVR based boards and *bossac* for SAM based
...
See [`{runtime.tools.*.path}` rules](package*index*json-specification.md#how-a-tools-path-is-determined-in-platformtxt)
...
`ARDUINO*USER*AGENT`: contains the name and version of the client used by the user in
...
[package index](package*index*json-specification.md), it might be cumbersome to use with manual installations. So we
...
`DISCOVERY*ID` must be replaced by a unique identifier for the particular discovery and `DISCOVERY*RECIPE` must be
...
where `PROTOCOL` must be replaced with the port protocol identifier and `VENDOR*ID:MONITOR*NAME` must be replaced with
...
[package index](package*index*json-specification.md). Since it might be cumbersome to use with manual installations, we
provide another syntax to ease development and beta testing:
...
`pluggable*discovery.*` or `pluggable*monitor.*` properties in `platform.txt` respectively) it will automatically
...
- `SETTING*NAME` and `SETTING*VALUE` are the port setting and the desired value
...
- `use*1200bps*touch` causes the selected serial port to be briefly opened at 1200 bps (8N1) before starting the upload.
- `wait*for*upload_port` causes the upload procedure to wait for the serial port to (re)appear before and after the
  upload. This is only used when `use*1200bps*touch` is also set. When set, after doing the 1200 bps touch, the
...
directive `debug.additional*config=CONFIG*PREFIX`. This rule will use the configuration under `CONFIG_PREFIX.*` to
...
**BOARD*ID.menu.MENU*ID.OPTION_ID=Text**, where **Text** is what is displayed under the "Processor" menu in the IDE's
GUI.
...
**VENDOR*ID:CORE*ID**. For example, if we want to define a board that uses the "arduino" core from the "arduino" vendor
we should write:
...
In the same way we can use a variant defined on another platform using the syntax **VENDOR*ID:VARIANT*ID**:
...
Note that, unlike core references, other resources (platform.txt, bundled libraries, programmers) are *not* inherited
...
Note that, unlike core references, referencing a tool recipe does *not* result in any other resources being inherited
from the referenced platform.
...
- **Arduino CLI**: (since 0.12.0) runs the script for any installed platform when Arduino CLI is in "interactive" mode.
  This behavior
  [can be configured](https://arduino.github.io/arduino-cli/latest/commands/arduino-cli*core*install/#options)
