...
- `board_manager`
  ...
  - `enable*unsafe*install` - set to `true` to allow installation of packages that do not pass the checksum test. This
...
- `library` - configuration options relating to Arduino libraries.
  - `enable*unsafe*install` - set to `true` to enable the use of the `--git-url` and `--zip-file` flags with
...
- `locale` - the language used by Arduino CLI to communicate to the user, the parameter is the language identifier in
  the standard POSIX format `<language>[*<TERRITORY>[.<encoding>]]` (for example `it` or `it*IT`, or `it_IT.UTF-8`).
...
- `sketch` - configuration options relating to [Arduino sketches][sketch specification].
  - `always*export*binaries` - set to `true` to make [`arduino-cli compile`][arduino-cli compile] always save binaries
...
- `build_cache` configuration options related to the compilation cache
  ...
  - `extra*paths` - a list of paths to look for precompiled artifacts if not found on `build*cache.path` setting.
  - `compilations*before*purge` - interval, in number of compilations, at which the cache is purged, defaults to `10`.
...
- `network` - configuration options related to the network connection.
  ...
  - `connection_timeout` - network connection timeout, the value format must be a valid input for
    ...
    wait indefinitely.
  - `cloud*api.skip*board*detection*calls` - if set to `true` it will make the Arduino CLI skip network calls to Arduino
...
configuration key names, with each component separated by `*`. For example, the `ARDUINO*DIRECTORIES_USER` environment
...
`ARDUINO*BOARD*MANAGER*ADDITIONAL*URLS` environment variables can be a list of space-separated URLs.

#### Example

Setting an additional Boards Manager URL using the `ARDUINO*BOARD*MANAGER*ADDITIONAL*URLS` environment variable:

```sh
$ export ARDUINO_BOARD_MANAGER_ADDITIONAL_URLS=https://downloads.arduino.cc/packages/package_staging_index.json
```

Setting multiple additional Boards Manager URLs using the `ARDUINO*BOARD*MANAGER*ADDITIONAL*URLS` environment variable:
...
1. Location specified by the `ARDUINO*CONFIG*FILE` environment variable
1. Location specified by the `ARDUINO*DIRECTORIES*DATA` environment variable
...
[arduino cli lib install]: commands/arduino-cli*lib*install.md
...
[arduino-cli config dump]: commands/arduino-cli*config*dump.md
...
[arduino-cli config init]: commands/arduino-cli*config*init.md
