...
semver string of a specific version. We've added the `installed*version` and `latest*version` to make more convenient
...
- `cc.arduino.cli.commands.v1.PlatformList`: you can use the already available gRPC method `PlatformSearch` to perform
  the same task. Setting the `all*versions=true` and `manually*installed=true` in the `PlatformSearchRequest` returns
...
The fields `toolchain*configuration` and `server*configuration` are no more generic `map<string, string>` but they have
...
**un**expanded `build*properties` you must set to `true` the field `do*not*expand*build_properties` in the
`CompileRequest`.
...
behavior can be [configured](https://arduino.github.io/arduino-cli/0.30/commands/arduino-cli*core*install/#options).
...
`LIBRARY*LOCATION*IDE*BUILTIN` has been renamed to `LIBRARY*LOCATION_BUILTIN`
...
now the properties have been renamed from `identification*prefs` to `identification*properties` and they are no longer
...
Also the option `library*manager*only` has been removed, the package manager is always initialized and platforms are
loaded.
...
- Enumerations now have their class name prefixed (for example the enumeration value `FLAT` in `LibraryLayout` has been
  changed to `LIBRARY*LAYOUT*FLAT`)
...
- Repeated responses are now in plural form (`identification*pref` -> `identification*prefs`, `platform` -> `platforms`)
...
- in `board details` command the following fields have been renamed:
  - `identification*pref` -> `identification*prefs`
  - `usbID` -> `usb_id`
  - `PID` -> `pid`
  - `VID` -> `vid`
...
specify the `--export-binaries` flag: `sketch.always*export*binaries` and `ARDUINO*SKETCH*ALWAYS*EXPORT*BINARIES`.
...
With the introduction of the `--git-url` and `--zip-file` flags the new config key `library.enable*unsafe*install` has
